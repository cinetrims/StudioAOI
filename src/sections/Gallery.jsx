      ...
      <div className="flex flex-col lg:flex-row items-center justify-between pt-16">
        ...
        <div className="lg:w-2/5 w-full mt-6 lg:mt-0 text-center lg:text-right">
          ...
          <p className="mt-3 text-sm font-normal font-clash text-black leading-relaxed">
            Our portfolio showcases a range of projects, from cozy living rooms to luxurious
            bedrooms, and everything in between. We have worked on residential projects, commercial
            spaces, and hospitality interiors, and have experience working with a variety of design
            styles.
          </p>
        </div>
      </div>
        ...
        {groupedImages.map(images => (
          <div className="grid grid-cols-3 grid-rows-3 gap-3 m-3 h-lvh">
            {images.map((image, index) => (
              ...
              >
                {/* Image */}
                <img src={image.imgSrc} alt={image.title} className="w-full h-full object-cover" />
                {/* Title on Image */}
                ...
                <button className="absolute bottom-0 right-0 p-2 rounded-full shadow-lg">
                  <img src={arrow} alt="Arrow Button" className="w-10 h-10" />
                </button>
              </div>
            ))}
          </div>
